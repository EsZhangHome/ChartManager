...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    public LineChartRenderer(LineDataProvider chart, ChartAnimator animator,
        ...
        mCirclePaintInner = new Paint(Paint.LINEAR_TEXT_FLAG);
    ...
    protected void drawCLineTag(Canvas c) {
        //绘制标签
        LineData lineData = mChart.getLineData();
        for (int i = 0; i < lineData.getDataSets().size(); i++) {
            LineDataSet lineDataSet = (LineDataSet) lineData.getDataSetByIndex(i);
            if (lineDataSet.isVisible() && lineDataSet.isDrawTags()) {
                List<Entry> entries = lineDataSet.getValues();
                Transformer trans = mChart.getTransformer(lineDataSet.getAxisDependency());
                MPPointD pointD = trans.getPixelForValues(entries.get(entries.size() - 1).getX(), entries.get(entries.size() - 1).getY());
                Paint paint = new Paint();
                paint.setTextSize(lineDataSet.getTagSize());
                paint.setStyle(Paint.Style.FILL);
                Rect rect = new Rect();
                paint.getTextBounds(lineDataSet.getLabel(), 0, lineDataSet.getLabel().length(), rect);
                float width = rect.width();
                float height = rect.height();
                rect.left = (int) (pointD.x - width - 20);
                rect.right = (int) (pointD.x - 20);
                rect.top = (int) (pointD.y - (height / 2));
                rect.bottom = (int) (pointD.y + (height / 2));
                paint.setColor(lineDataSet.getTagBgColor());
                c.drawCircle((float) rect.right - (width / 2), (float) pointD.y, (width / 2f + 5), paint);
                paint.setColor(Color.TRANSPARENT);
                c.drawRect(rect, paint);
                paint.setColor(lineDataSet.getTagColor());
                c.drawText(lineDataSet.getLabel(), rect.left, rect.bottom, paint);
            }
        }
    }
