...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    /**
     * 指示是否启用捏缩放的标志。如果为真，x 和 y 轴都可以用 2 个手指缩放，如果为假，x 和 y 轴可以分别缩放
     */
    ...
    /**
     * 设置图表周围的最小偏移量（填充），默认为 15
     */
    protected float mMinOffset = 15.f;

    /**
     * 指示图表是否应在旋转后保持在同一位置的标志。默认为假。
     */
    ...
    /**
     * 返回一个可回收的 MPPointF 实例。返回提供的条目在图表视图中的位置（以像素为单位），如果提供的条目为空，则返回空。
     *
     * @param e
     * @return
     */
    ...
    /**
     * 设置图表上最大可见绘制值的数量，仅在启用 setDrawValues() 时激活
     *
     * @param count
     */
