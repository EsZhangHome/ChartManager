...
public abstract class AxisBase extends ComponentBase {
    ...
    /**
     * 当为真时，轴标签由“粒度”属性控制。
     * 当为 false 时，轴值可能会重复。如果两个相邻的轴值四舍五入到相同的值，
     * 就会发生这种情况。如果使用粒度，可以通过减少可见的轴值来避免这种情况。
     */
    ...
    /**
     * `axisMinimum` 的额外间距将添加到自动计算的 `axisMinimum` 中
     */
    protected float mSpaceMin = 0.f;

    /**
     * `axisMaximum` 的额外间距将添加到自动计算的 `axisMaximum` 中
     */
